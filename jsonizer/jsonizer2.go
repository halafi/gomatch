...
func main() {
	...
	tokenFile, patternsFile, t := string(tokFile),string(pFile),string(tFile)
	//Preprocessing
	matchesPerLine := make(map[int][]string)
	//occurences := make(map[string][]int)
	p := make([]string, 0) //patterns
	r := make([]string, 0) //regexes
	...
	for i := range lines {
		...
		for j := range words {
			if words[j][0] == '<' && words[j][len(words[j])-1] == '>' {
				r = addWord(r, words[j])
			}
		}
	}
	...
	for n := range lines { //for each log line
		...
		current := 0
		for pos := range words { //prochazime jeden radek po slovech
			for getTransition(current, words[pos], ac) == -1 && len(getTransitionTokens(current, ac)) == 0 && s[current] != -1 { //we go down the supply path
				current = s[current]
			}
			...
			if len(getTransitionTokens(current, ac)) > 0 { //we can move in 'ac' by some regex
				...
				for r := range tokens {
					tokenToMatch := getToken(tokenFile, getWord(1, len(tokens[r])-2, tokens[r]))
					regex := regexp.MustCompile(tokenToMatch)
					if regex.MatchString(words[pos]) { //we got one match
						passableTokens = addWord(passableTokens, tokens[r])
					}
				}
				if len(passableTokens) > 1 {
					log.Fatal("we can move state by multiple regexes")
				} else if len(passableTokens) == 1{
					current = getTransition(current, passableTokens[0], ac)
				}
			}
			if len(passableTokens) == 0 && getTransition(current, words[pos], ac) != -1 { //move by a word
				current = getTransition(current, words[pos], ac)
			}
			_, ok := f[current]
			if ok { //we got one or more matches
				for i := range f[current] { // for each pattern that current state is final for
					if isMatch(lines[n], p[f[current][i]], tokenFile) { //if (current pattern[i] is match for current line)
						currentMatchText := matchString(lines[n], p[f[current][i]], tokenFile)
	...
	if len(logLineWords) != len(patternWords) {
		...
		for i := range logLineWords {
			if logLineWords[i] != patternWords[i] {
				tokenToMatch := getToken(tokenFile, getWord(1, len(patternWords[i])-2, patternWords[i]))
				regex := regexp.MustCompile(tokenToMatch)
	...
	for i := range logLineWords {
		if logLineWords[i] != patternWords[i] {
			...
			if regex.MatchString(logLineWords[i]) {
				...
				output = output + getWord(1, len(patternWords[i])-2, patternWords[i]) +" = "+logLineWords[i]
				tokens = true
			}
		}
	}
